...
func buildMetricRows(ctx context.Context, costUsageData *costexplorer.GetCostAndUsageOutput, keyQuals map[string]*proto.QualValue) []MetricRow {
	logger := plugin.Logger(ctx)
	logger.Trace("buildMetricRows")
	var rows []MetricRow
...
// MetricRow is the flattened, aggregated value for a metric.
...
func (row *MetricRow) setRowMetrics(metrics map[string]*costexplorer.MetricValue) {
...
func getStartDateForGranularity(granularity string) time.Time {
	switch granularity {
	case "DAILY", "MONTHLY":
		// 1 year??
